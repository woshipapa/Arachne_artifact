...
Timing log visualization (local experiments):
Use `dynamic_flex_exp_log/new_visualize_all_timer_rank.py` to parse `timing_rank*.log`,
average per-rank timelines, and emit JSON summaries plus Gantt charts.
Configure `USE_PARENT_ITERATION_TIME`, `model`, `resolution`, `base_log_dir`, and `framework`
inside the script to point at the right log directory and overhead handling.
