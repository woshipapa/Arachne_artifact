...
def estimate_hunyuan_vae_encoder_flops(
    ...
    """
    精确计算 HunyuanVideo VAE Encoder 的 Forward FLOPs (已修正 Batch Size)。
    """
    total_flops = 0.0
    ...
    for i, target_out_channel in enumerate(block_out_channels):
        ...
        if add_downsample:
            ...
            # 修正: 这里的 Downsample Conv 计算也包含了 Helper 里的 batch_size
    ...
    for _ in range(mid_block_layers):
        ...
        if mid_block_add_attention:
            ...
            # 修正: Linear Projections 需乘以 batch_size
            # 4 * B * S * C^2
            mid_flops += 4 * 2 * batch_size * seq_len * (mid_channels**2)
            
            # 修正: Attention Matrix Ops 需乘以 batch_size
            # 4 * B * S^2 * C
