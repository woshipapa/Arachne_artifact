...
class FlopsSimulator:
    ...
    def evaluate_rank_flops(self, global_rank_id, task_list):
        """
        计算指定 Global Rank 的总 FLOPS，并返回详细列表
        Returns: (total_flops, detailed_task_list)
        """
        total_flops = 0.0
        detailed_records = [] # 新增：用于记录每个任务的详情
...
class ExperimentAnalyzer:
    ...
    def run(self):
        ...
        for y_path in sorted(yaml_files):
            # 从文件名解析 Iteration ID: schedule_0.yaml -> "0"
...
if __name__ == "__main__":
    ...
    # schedule：用于找 schedule yaml
    # schedule_type = "megatron-lm"
    # schedule_type = "megatron-lm"
    schedule_type = "flex_sp"
    # schedule_type = "genetic"
    # framework：用于找日志目录
    # 通常和 schedule_type 一样，但不强制
    # framework = "flex_sp"
    # framework = "arachne"
    if schedule_type == "megatron-lm":
        framework = "megatron-lm"
    elif schedule_type == "genetic":
        framework = "arachne"
    elif schedule_type == "flex_sp":
        framework = "flex_sp"
    # 日志根目录
    base_log_dir = "baseline_log"
    # base_log_dir = "dynamic_flex_exp_log"
    max_frames = ""
    output_csv = "mfu_summary.csv"
