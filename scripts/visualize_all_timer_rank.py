...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Parse per-rank timing_rank*.log into per-iteration / per-rank "
                    "time & idle JSON (analysis_summary.json + iteration_times_summary.json)."
    )
    # Recommended: point directly at the folder that contains timing_rank*.log,
    # e.g. the training output  logs/handler_logs/<MODEL>/<RES>/<FRAMES>/<SCHED>/
    parser.add_argument("--log-dir", default=None,
                        help="Directory directly containing timing_rank*.log. "
                             "Overrides the legacy base/framework/model/resolution layout.")
    parser.add_argument("--out-dir", default=None,
                        help="Output dir for figures + JSON (default: <log-dir>/figures).")
    # Legacy layout (used only when --log-dir is NOT given); defaults reproduce the bundle.
    parser.add_argument("--base-log-dir", default="baseline_log")
    parser.add_argument("--framework", default="flex_sp")
    parser.add_argument("--model", default="hunyuan-129")
    parser.add_argument("--resolution", default="360p")
    args = parser.parse_args()

    if args.log_dir:
        log_search_path = os.path.join(args.log_dir, "timing_rank*.log")
        output_figure_dir = args.out_dir or os.path.join(args.log_dir, "figures")
    else:
        log_search_path = os.path.join(args.base_log_dir, args.framework, args.model,
                                       args.resolution, "timing_rank*.log")
        output_figure_dir = args.out_dir or os.path.join(args.base_log_dir, args.framework,
                                                         args.model, args.resolution, "figures")
