...
def parse_and_group_runs(log_search_path):
    ...
    解析 Tree 风格日志。
    逻辑核心：
    1. 遇到 "Stage 'iteration-X'" 视为新的一轮采样的【开始信号】。
    2. 此时将【上一轮】缓存的数据存入 all_runs_data。
    3. 真正的 Iteration ID 取自 'iteration-X' 中的 X，而非日志行的 [Iter Y]。
    4. 即使日志发生 Reset (Iter 计数归零)，也不清空之前已存的数据，而是继续追加。
    """
    all_runs_data = defaultdict(lambda: defaultdict(list))
    log_files = glob.glob(log_search_path)
    ...
    for log_file in sorted(log_files):
        ...
        # 状态变量 (Context)
        current_true_iter_id = None  # 当前归属的 Iteration ID (来自 iteration-X)
        current_rank = -1
        current_run_buffer = []      # 缓存当前这一轮 (one sample) 的子阶段
        
        # 辅助函数：将缓存存入总表
        def flush_buffer():
            nonlocal current_run_buffer
            # 只有当 ID 有效且 Buffer 非空时才保存
        ...
        with open(log_file, 'r') as f:
            for line in f:
                ...
                if match:
                    ...
                    # 如果 Rank 变了 (极少见，除非单文件多 Rank)，先存旧数据
                    ...
                    # 1. 这是一个 "父节点" (iteration-X)吗？
                    #    如果是，说明上一轮采样结束了，新的一轮开始了。
                    if stage_name.startswith('iteration-'):
                        # [Step A] 先把上一轮收集好的子阶段存起来
                        flush_buffer()
                        
                        # [Step B] 更新当前的 True Iteration ID
                        try:
                            # 从 'iteration-8' 中提取 8
                        ...
                        # 注意：父节点本身的耗时我们这里不存入 buffer，
                        # 因为 buffer 只存子阶段用于画甘特图。
                        # 如果你需要总耗时，可以在这里单独处理。

                    # 2. 这是一个 "子节点" (VAE, DiT 等)吗？
                    else:
                        # 只有当我们处于一个有效的 Iteration 上下文中时才收集
        ...
        # 文件读完后，别忘了保存最后一轮滞留在 buffer 里的数据
        flush_buffer()
...
if __name__ == "__main__":
    model, resolution = "hunyuan-129", "360p"
