...
class DumpConfig:
    ...
    enable: bool = True
    # limits to keep dumps from growing unbounded
    max_list_elems: int = 64          # keep at most the first N list/tuple elements
    max_dict_items: int = 128         # keep at most the first N dict keys
    max_str_len: int = 10_000         # truncate over-long strings
    save_tensor_cpu: bool = False     # True: move tensors to CPU first, lowering GPU use and failure risk
    tensor_save_format: str = "pt"    # "pt" or "pt+np"
    image_format: str = "png"         # "png" / "jpg"
    print_summary: bool = True
    # whether each dump also writes a summary.json
...
class UniversalDumper:
    ...
    def _save_any(self, obj: Any, out_dir: str, name: str) -> Dict[str, Any]:
        ...
        if Image is not None and isinstance(obj, Image.Image):
            # normalise the mode so dumps stay comparable (switch to "RGBA" if needed)
            try:
                if obj.mode != "RGB":
                    obj = obj.convert("RGB")
            except Exception:
            ...
            # PNG is strongly preferred: lossless and comparable pixel by pixel
            ext = self.cfg.image_format.lower()
            if ext in ("jpg", "jpeg"):
                # alternatively, reject jpg outright and force png
                # ext = "png"
                pass
            ...
            try:
                if ext in ("jpg", "jpeg"):
                    obj.save(path, format="JPEG", quality=95)  # lossy; unsuitable for strict comparison
                else:
...
# assumes these two classes previously lived in the same file
    ...
    def get(cls, cfg=None):
        ...
        with cls._lock:
            if cls._instance is None:
                ...
                # optionally append the pid to root_dir so concurrent processes do not
                # collide on one directory; comment out to share a single root_dir
                try:
                    pid = os.getpid()
                ...
                # a later caller passing a different cfg is ignored here (raising is also an option)
                if cfg is not None:
                    new_fp = repr(cfg)
                    if cls._cfg_fingerprint != new_fp:
                        # not raising is fine, but logging it is useful
                        print(
...
class UniversalLoader:
    ...
    def find_dump(self, data_id: int, stage: str, branch: str = None, pick: str = "latest"):
        ...
        for d in dirs:
            ...
            if branch is None and m.get("branch") is not None:
                # strict by default: branch=None matches only branch=None. Relax here if
                continue
            matched.append((d, m))
    ...
    def _load_saved(self, saved: dict):
        ...
        if kind == "seq_dir":
            ...
            # the original may have been a list or a tuple; restore a list (the common case)
            return elems
        ...
        if kind == "repr_txt":
            # not restorable: return its repr text
            with open(saved["path"], "r", encoding="utf-8") as f:
                return f.read()
