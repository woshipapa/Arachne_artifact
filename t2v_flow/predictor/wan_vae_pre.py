...
class _TileEncoderModelLoader:
    ...
    def load(self, directory: str, name: str) -> bool:
        try:
            ...
            print(f"[OK] [Predictor] base tile model '{name}' loaded.")
            return True
        except FileNotFoundError as e:
            print(f"[ERROR] failed to load the base tile model; file not found: {e}")
            return False
    ...
    def _get_features(self, df_tile: pd.DataFrame):
        if self.ml_columns is None: raise RuntimeError("The base model has not been loaded.")
...
class VaeSystemPredictor:
    ...
    def _load_models(self, model_dir: str, base_model_name: str, system_model_name: str):
        if not self.base_tile_model.load(model_dir, base_model_name):
            raise FileNotFoundError("Could not load the base tile model; the predictor cannot initialise.")
        ...
        try:
            ...
            print(f"[OK] [Predictor] system-level model '{system_model_name}' loaded.")
        except FileNotFoundError as e:
            print(f"[ERROR] failed to load the system-level model; file not found: {e}")
            raise e
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Predict total VAE compute latency with the layered simulation model.")
    parser.add_argument('--bs', type=int, default=1, help='batch size')
    parser.add_argument('--f', type=int, default=113, help='number of frames')
    parser.add_argument('--h', type=int, default=1280, help='frame height')
    parser.add_argument('--w', type=int, default=720, help='frame width')
    parser.add_argument('--sp', type=int, default=5, help='tile parallel size')
    ...
    try:
        ...
        print(f"  - bs: {args.bs}\n  - f: {args.f}\n  - h: {args.h}\n  - w: {args.w}\n  - sp: {args.sp}")
        print("-" * 50)
        print(f"Predicted total VAE latency:")
        print(f"  -> {total_time:.6f} s")
        print("="*50)
    
    except FileNotFoundError as e:
        print(f"\nError: failed to load a model file. Make sure the base and system models")
        print(f"have been trained and live under '{MODEL_DIR}'.")
