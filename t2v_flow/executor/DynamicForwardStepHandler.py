...
class DynamicForwardStepHandler(metaclass=SingletonMeta):
    """
    单例的训练执行器。
    它持有一个 Planner 实例，并根据其计划来调度 forward_step。
    """
    ...
    def _send_control_msg(self, dst: int, msg: dict, tag: int):
        """辅助函数：手动序列化并通过 GLOO 组发送一个带 Tag 的控制消息字典。"""
        try:
    ...
    def _recv_control_msg_from_size(self, src: int, size_tensor: torch.Tensor, tag: int) -> dict:
        """
        辅助函数：根据已知大小，通过 GLOO 组接收控制消息。
        主要由 Broker 的轮询探查循环使用。
        """
        try:
    ...
    def _send_prepacked_data(self, dst: int, metadata: list, packed_cpu_buffer: torch.Tensor, tag: int):
            """[带日志] 辅助函数：将预打包好的 metadata 和 buffer 发送给目标 rank。"""
            try:
    ...
    def _send_prepacked_data_error(self, dst: int, error: str, tag: int) -> None:
            """通过 DATA 通道发送错误帧。约定：size<0 表示错误，绝对值为错误字符串字节数。"""
            try:
    ...
    def _recv_prepacked_data(self, src: int, tag: int) -> Tuple[list, torch.Tensor]:
            """[带日志] 辅助函数：从源 rank 接收预打包好的 metadata 和 buffer。"""
            try:
    ...
    def _publisher_worker_loop(self):
        """【发布者线程】从队列获取任务，打包，卸载到CPU，并发布给 Broker。"""
        while True:
    ...
    def _publish_listener_loop(self):
        """【Broker 专属迭代同步线程】只处理 PUBLISH 请求。"""
        while True:
    ...
    def update_and_setup_for_iteration(self, plan_filepath: str, curr_iter: int):
        """
        [新的入口方法] 在每个Megatron iteration开始时调用。
        负责更新计划、创建通信组、并为当前rank准备好任务列表。
        """
    ...
    def get_mock_batch_8422(self):
        """
        根据 Global Rank 为不同组的 GPU 生成指定帧数的 mock 数据。
        同时，每个组的 batch size 和其他 tensor 的 shape 与原始代码保持一致。

        - Ranks [0, 1]:      13 帧,  bs=6, prompt_embeds=(bs, 112, 4096)
        - Ranks [2, 3]:      57 帧,  bs=4, prompt_embeds=(bs, 115, 4096)
        - Ranks [4, 5, 6, 7]: 89 帧,  bs=2, prompt_embeds=(bs, 120, 4096)
        - Ranks [8-15]:     129 帧, bs=2, prompt_embeds=(1, 124, 4096)
        """
        global_rank = dist.get_rank()
    ...
    def get_mock_batch_dynamic_frames(self):
        """
        根据当前迭代次数 self.iter 动态生成 mock batch。
        - Batch size 每 2 个 iteration 增加 1, 从 1 增长到 8。
        - 分辨率固定为 1280x720。
        - 帧数 (frame) 随 rank 动态变化。
        """
        ...
        if not (0 <= global_rank < 8):
            raise ValueError(f"此方法仅为 rank 0-7 设计, 当前 rank 为 {global_rank}")
    ...
    def get_mock_batch_all_129_frames(self):
        """
        为所有 ranks (0-15) 都生成 129 帧的 mock 数据。

        所有 tensor 的 shape 与之前 129 帧的配置完全一致：
        - bs=2
        - prompt_embeds=(1, 124, 4096)
        - clip_text_embed=(1, 768)
        """
        global_rank = dist.get_rank()
        local_rank = global_rank % torch.cuda.device_count()
        torch.cuda.set_device(local_rank)
    ...
    def get_bucket_data_list(self):
        """
        为每个帧数和分辨率的组合，精确计算最大允许的batch_size，
        并生成数据列表。
        """
        width = 1280
        height = 720
        ...
        try:
            ...
            print("[警告] mpu 库不可用，sp (并行数) 将使用默认值 4。")
            sp = 4                   
        ...
        print(f"总共生成了 {len(data_list)} 个数据组合。")
        print("示例 (max_bs, num_frames, C, H, W):")
        if len(data_list) > 6:
    ...
    def get_per_rank_schedules(self, world_size: Optional[int] = None):
        """
        [新功能] 获取每个 rank 的完整任务执行序列。

        这个方法是一个委托，它调用内部 planner 实例的同名方法来完成实际工作。
        这使得外部代码无需直接与 planner 交互，保持了良好的封装性。

        Args:
            world_size (int, optional): 分布式环境中的总进程数。

        Returns:
            Dict[int, List[Task]]: 一个字典，键是 rank 号，值是该 rank 的任务列表。
        """
