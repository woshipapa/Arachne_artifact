        ...
        while len(completed_tasks) < len(proc_times):
            ...
            for task_key in list(ready_queue):
                ...
                if base_proc_time == float('inf'):
                    raise ValueError(f"Task {task_key} has an invalid SP value ({k}), making its execution time infinite.\n"
                                     f"Check the genetic algorithm's chromosome generation and mutation logic.")
                # ========================================================
            ...
            if not scheduled_this_loop and ready_queue:
                raise RuntimeError("Scheduling deadlock: no task is schedulable but the queue is not empty.")
    else:
        raise NotImplementedError(f"Strategy '{priority_strategy}' is not implemented.")
