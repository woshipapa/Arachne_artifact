...
class SchedulePool:
    def __init__(
        self,
        num_workers: int = 1, # 这里不再需要多个 worker，参数保留但实际上我们只用主进程
    ...
    def submit(self, iteration: int):
        """
        修改后的 submit 方法：直接同步执行任务，不再放入队列。
        这确保了没有多线程干扰，可以让内部的 joblib 安全地 fork/spawn 多进程。
        """
        ...
        if ENABLE_OPTIMAL_SEARCH:
            try:
                if OPTIMAL_MODE == "pure_sp":
                    ...
                    optimal_result = solve_topology_baseline(
                        ...
                        n_gpus=self.n_gpus,
                        topology_model=topo,
                        time_limit_sec=60, # 给它足够的时间展示下降过程
                        log=True
                    )
                    ...
                    optimal_result = solve_dfs_topology(
                        ...
                        n_gpus=self.n_gpus,
                        topology_model=topo,
                        time_limit_sec=60, # DFS 需要较多时间来回溯
                        log=True
                    )
                    topology_aware = True
            ...
            optimal_dump = {
                "meta": {
                    ...
                    "time_limit_sec": 1000 if OPTIMAL_MODE != "brute_force" else 60, # 记录实际设定的时间
                    "solver": solver_name,
...
if __name__ == "__main__":
    ...
    pool = SchedulePool(
        num_workers=1,  # 虽然这个参数没用了，但保留接口兼容
        n_gpus=NGPUS,
        machine_size=8,
