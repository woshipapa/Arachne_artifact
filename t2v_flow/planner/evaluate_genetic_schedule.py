...
def get_critical_path_priority(task_key: Tuple[int, str], predecessors: Dict, proc_times: Dict) -> float:
    ...
    if not successors:
        return proc_times.get(task_key, float('inf')) # 使用 .get() 增加健壮性
...
def generate_trace_from_chromosome(tasks_input: List[Dict], chromosome: List[int], total_gpus: int, 
    ...
    for task_def in tasks_input:
        ...
        # ==================== Bug修正区域 ====================
        # 正确地为 VAE 和 DiT 填充 proc_times 和 sp_map
    ...
    # --- 策略3: 组合优化 (局部穷举) ---
        ...
        raise ValueError(f"未知的优先级策略: {priority_strategy}")
...
if __name__ == '__main__':
    ...
    # ===============================================================
    # 2. 请在这里选择您想运行和评估的策略！
    # 选项: "critical_path", "max_concurrency", "exhaustive_combination"
    # ===============================================================
    PRIORITY_STRATEGY = "critical_path"
