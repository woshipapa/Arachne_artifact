...
import torch
# from bucket_utils import gather_micro_batches_across_gpus
...
class VariableVideoBatchSampler(DistributedSampler):
    ...
    def __init__(self,
        ...
        self.total_samples = len(self.dataset.data_list)
    ...
    def __iter__(self) -> Iterator[list[int]]:
        ...
        print(f'bucket_id_access_order is {len(bucket_id_access_order)}')        
        ...
        for i in range(start_iter_idx, num_iters):
            ...
            # debug_list = gather_micro_batches_across_gpus(cur_micro_batch)
            # if dist.get_rank() == 0:
    ...
    def group_by_bucket(self) -> dict:
        ...
        self.runtime_bucket.clear()
        self.seqlen2cnt.clear()
        for clip in data_list:
                ...
                if len(valid_range) != 0:
                    ...
                    count = valid_range[1]-valid_range[0]
                
       
                    
                key = self.bucket.get_bucket_id(count,clip.height,clip.width,clip.fps)
                if key is None:
