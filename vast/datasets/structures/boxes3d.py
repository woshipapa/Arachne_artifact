...
class Boxes3D(BaseStructure):
    ...
    def angles(self):
        return (
            self.tensor[:, 6:9]
            if self.rot_axis == -1
            else self.tensor[:, 6 + self.rot_axis, None]
        )

    def convert_to(self, dst_mode, rot_mat=None):
        return boxes3d_utils.convert_boxes3d(
            self, src_mode=self.mode, dst_mode=dst_mode, rot_mat=rot_mat
        )
    ...
    def corners2d(self, cam_intrinsic=None, bev=False):
        ...
        if bev:
            ...
            return boxes3d_utils.boxes3d_to_corners2d(
                self.tensor, self.rot_axis, cam_intrinsic
            )
    ...
    def boxes2d(self, cam_intrinsic):
        ...
        if not isinstance(cam_intrinsic, torch.Tensor):
            cam_intrinsic = self.tensor.new_tensor(cam_intrinsic)
        return boxes3d_utils.boxes3d_to_boxes2d(
            self.tensor, self.rot_axis, cam_intrinsic
        )
    ...
    def in_range_bev(self, box_range):
        return points3d_utils.points3d_in_range_bev(
            self.tensor[:, :3], box_range, self.bev_axis
        )

    def limit_angles(self, offset=0.5, period=np.pi):
        self.tensor = boxes3d_utils.limit_angles(
            self.tensor, self.rot_axis, offset, period
        )
    ...
    def rotate(self, angle, rot_mat_t=None):
        ...
        self.tensor = boxes3d_utils.rotate_boxes3d(
            self.tensor, rot_mat_t, angle, self.rot_axis
        )

    def flip(self, bev_direction="horizontal"):
        self.tensor = boxes3d_utils.flip_boxes3d(
            self.tensor, self.flip_axis, self.rot_axis, bev_direction
        )
