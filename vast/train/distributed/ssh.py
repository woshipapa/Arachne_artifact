...
def ssh_copy_id(ips, username, password, port):
    ...
    for i, client in enumerate(clients):
        cmd = f"cat $HOME/.ssh/id_rsa.pub >> {authorized_keys}"
        stdin, stdout, stderr = client.exec_command(cmd)
        print(ips[i], stdout.readlines())
    with open(run_path, "w") as fn:
        fn.write("#!/bin/bash\n\n")
        fn.write(f"cp {authorized_keys} $HOME/.ssh/\n")
        for ip in ips:
            cmd = f'ssh-copy-id -i $HOME/.ssh/id_rsa.pub {username}@{ip} -p {port} -o "StrictHostKeyChecking no"'
            fn.write(f"{cmd}\n")
    for i, client in enumerate(clients):
        cmd = f"sh {run_path}"
        stdin, stdout, stderr = client.exec_command(cmd)
