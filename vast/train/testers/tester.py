...
from vast.datasets import DefaultCollator, load_dataset
...
class Tester:
    ...
    def load(cls, config_or_path):
        ...
        tester.prepare(
            ...
            models=config.models.test
            if hasattr(config.models, "test")
            else config.models,
        )
        return tester
    ...
    def print_step(self):
        if self.cur_step % self.log_interval == 0:
            ...
            eta_sec = max(
                0, time_cost / self.log_interval * (self.epoch_size - self.cur_step)
            )
