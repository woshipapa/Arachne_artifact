...
class BaseScheduler(object):
    def __init__(
        self, epoch_size, max_epochs, max_steps, start_value=1.0, end_value=0.0
    ):
...
class CosineScheduler(BaseScheduler):
    ...
    def get_factor(self, num_update):
        start_factor = (
            0.5
            * self.weight
            * (1 + math.cos(math.pi * (num_update - 1) / (self.max_steps - 1)))
        )
        return start_factor, 1 - start_factor
...
class StepScheduler(BaseScheduler):
    def __init__(
        self, step_factor=0.1, step_size_list=None, step_epoch_list=None, **kwargs
    ):
