...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def __init__(self,
        ...
        for i, params_group in enumerate(self.bit16_groups):
            for param in params_group:
                ...
                self.id2name[count] = self.param_names[param]
                # logger.info(f'[Rank {dist.get_rank()}] id is {count}, param_name is {self.id2name[count]}')
    ...
    def reduce_independent_p_g_buckets_and_remove_grads(self, param, i):
        ...
        # if dist.get_rank() == 0:
            # logger.info(f'[Rank {dist.get_rank()}] param_id {param_id},param_name is {self.id2name[param_id]} grad_func {torch.isnan(grad_reduc).any()}')
        if self.contiguous_gradients:
