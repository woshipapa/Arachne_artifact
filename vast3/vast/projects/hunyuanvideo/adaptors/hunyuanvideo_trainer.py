...
class HunYuanVideoTrainer(Trainer):
    ...
    def forward_step(self, batch_dict):
        with self.accelerator.autocast():
            ...
            global_timer.start(f"hunyuan_vae_forward_bs_{batch_size}_f_{num_frames}_h_{height}_w_{width}")
            latents = self.forward_vae(images) * self.vae.config.scaling_factor
            global_timer.stop(f"hunyuan_vae_forward_bs_{batch_size}_f_{num_frames}_h_{height}_w_{width}")
            # latents = F.pad(latents,(0,0,0,0,0,1), mode='constant',value=0)
            ...
            # print(f'[Rank {torch.distributed.get_rank()}] timesteps = {timesteps}, sigmas = {sigmas}') diff rank have the same timestep(seed same)
            ...
            if "ref_images" in batch_dict:
                ...
                # logger.info(f'add ref_images shape is {noisy_model_input.shape}')
            ...
            if "first_ref_image" in batch_dict:
                ...
                # logger.info(f'add first_ref_images shape is {noisy_model_input.shape},mask_shape is {mask.shape},conditional_latents is {conditional_latents.shape}')
        ...
        with self.accelerator.autocast():
            ...
            # print(f'[Rank {torch.distributed.get_rank()}]input transformer block {latent_model_input.shape}, prompt_embeds {prompt_embeds.shape}')
            dit_log = f"hunyuan_dit_forward_bs_{latent_model_input.shape[0]}_f_{latent_model_input.shape[2]}_h_{latent_model_input.shape[3]}_w_{latent_model_input.shape[4]}"
