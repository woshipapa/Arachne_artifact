...
class VASTBASELoss(nn.Module):
    def __init__(
        ...
        self.type = type
        discretizer = ZeroSNRDDPMDiscretization(num_timesteps=num_timesteps, shift_scale=shift_scale)
    ...
    def add_noise(self, input_):
        ...
        if self.uniform_sampling:
            ...
            timesteps = torch.randint(start_idx, end_idx, (input_.shape[0],)).to(input_.device)
        else:
            timesteps = torch.randint(0, self.num_timesteps, (input_.shape[0],)).to(input_.device)

        noise = torch.randn_like(input_)
        if self.offset_noise_level > 0.0:
            offset_noise = append_dims(torch.randn(input_.shape[0]).to(input_.device), input_.ndim)
        ...
        noised_input = input_.float() * alphas_cumprod_sqrt + noise * (1 - alphas_cumprod_sqrt**2) ** 0.5
