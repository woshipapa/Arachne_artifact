...
class Accelerator:
    ...
    **Available attributes:**
    ...
    def prepare(self, *args, device_placement=None):
        ...
        # add deepseedEngine to parallelManager

        from .sp_utils import ParallelManager
        ParallelManager.set_engine(result[1])        

        # from vast.train.utils import logger
        # logger.info(f'accelerator prepare {self._models[0].seq_parallel_group}.....................................................................,')
    ...
    def _prepare_deepspeed(self, *args):
        import deepspeed
        # logger.info(f'accelerate#prepare_ds  args = {args}')
        ...
        # print(f'config = {deepspeed_plugin.deepspeed_config}, dp_size = {deepspeed_plugin.deepspeed_config["data_parallel_size"]}')
        self.dp_size = 1 if deepspeed_plugin.deepspeed_config.get('data_parallel_size') is None else deepspeed_plugin.deepspeed_config.get('data_parallel_size') 
        self.sp_size = 1 if deepspeed_plugin.deepspeed_config.get('sequence_parallel_size') is None else deepspeed_plugin.deepspeed_config.get('sequence_parallel_size')
        # print(f'deepspeed_plgin {deepspeed_plugin}')
        is_dataloader_present = any(isinstance(obj, torch.utils.data.DataLoader) for obj in args)
        result = [
            # prepare_data_loader--->batchSamplerShard
        ...
        if deepspeed_plugin.is_auto("train_micro_batch_size_per_gpu"):
            if is_dataloader_present:
                ...
                logger.info(f'batch_sizes {batch_sizes},batch_size_per_device {batch_size_per_device}')
        ...
        if model is not None:
            ...
            # assert engine.seq_parallel_group is not None
            # assert engine.data_parallel_group is not None
            # import torch.distributed as dist
            # sp_size =4
            # dp_size =2
            # assert dist.get_world_size(group=engine.seq_parallel_group) == sp_size
            # assert dist.get_world_size(group=engine.data_parallel_group) == dp_size
            # assert dist.get_world_size() == sp_size * dp_size
            # from vast.train.utils import logger
            # logger.info(f'seq_group: {engine.seq_parallel_group},   data_group: {engine.data_parallel_group}')
            ...
            # print(f'before {self._models}')    
            self._models.append(engine)
            # print(f'after {self._models}')
            # print(f'{engine.seq_parallel_group,  engine.data_parallel_group}')
            if optimizer is not None:
    ...
    def prepare_data_loader(
        ...
        prepared_data_loader = prepare_data_loader(
            ...
            sequnce_parallel_size = self.sp_size,
