...
class Debugger:
    ...
    @classmethod
    def register_debug_hook(cls, module, tag=''):

        def build_hook(name):

            def begin_hook(module, grad_output):
                # nonlocal cnt
                if len(grad_output) == 0:
                    print(f"[Rank {torch.distributed.get_rank()}]_[module {name}] no grad_output")
                    return
                output_nan = grad_output[0].isnan().any()
                print(f"{tag} [Rank {torch.distributed.get_rank()}]_[start module {name}]  len is {len(grad_output)},grad output: {output_nan} ")
                if not output_nan:
                    print(f"{tag} [Rank {torch.distributed.get_rank()}]_[start module {name}], max value is {grad_output[0].max()}, min_value is {grad_output[0].min()}" )
                if not cls.find_nan and output_nan:
                    cls.find_nan = True
                    torch.save(grad_output,'start_nan_tensor.pt')
                    print(f'module {name} is saved ')
                

            def end_hook(module, grad_input, grad_output):
                # nonlocal cnt
                if len(grad_output) == 0 or len(grad_input) == 0:
                    print(f"[Rank {torch.distributed.get_rank()}]_[module {name}] no grad_output or grad_input")
                    return
                print(f"{tag} [Rank {torch.distributed.get_rank()}]_[end module {name}] grad_output len is {len(grad_output)}, grad output: {grad_output[0].isnan().any()}, grad_input len is {len(grad_input)}, grad input: {grad_input[0].isnan().any()}  ")
                output_nan = grad_output[0].isnan().any()
                input_nan = grad_input[0].isnan().any()
                if not output_nan:
                    print(f"{tag} [Rank {torch.distributed.get_rank()}]_[end module {name}]_[grad_output], max value is {grad_output[0].max()}, min_value is {grad_output[0].min()} ")
                if not input_nan:
                    print(f"{tag} [Rank {torch.distributed.get_rank()}]_[end module {name}]_[grad_input], max value is {grad_input[0].max()}, min_value is {grad_input[0].min()} ")    
                if not cls.find_nan and (output_nan or input_nan):
                    cls.find_nan = True
                    print(f'{grad_input[0]}')
                    torch.save([grad_output,grad_input],f'end_nan_tensor.pt')
                    print(f'module {name} is saved ')
            
            def end_forward_hook(module, input, kwargs,output):
                torch.save([input,output,kwargs], f'saved_tensor/{name}_rank{torch.distributed.get_rank()}.pt')
               

            return begin_hook, end_hook, end_forward_hook

    
        for name, m in module.named_modules():
            begin_hook, end_hook, end_forward_hook = build_hook(name)
            m.register_full_backward_pre_hook(begin_hook)
            m.register_full_backward_hook(end_hook)
            # m.register_forward_hook(end_forward_hook, with_kwargs=True)
...
class ParallelManager():
    ...
    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            # 只有在第一次访问时才创建实例
            cls._instance = super(ParallelManager, cls).__new__(cls, *args, **kwargs)
    ...
    def get_seq_parallel_rank():
        if ParallelManager.is_enable_seq_parallel():
            group = ParallelManager.engine.seq_parallel_group
            ranks_in_group = dist.get_process_group_ranks(group)
            global_rank = dist.get_rank()

            if global_rank in ranks_in_group:
                return ranks_in_group.index(global_rank)
            else:
                return None
    ...
    def get_seq_parallel_rank():
        """
        Gets the rank of the current process within the sequence parallel group.
        Returns 0 if sequence parallelism is not enabled.
        """
        if ParallelManager.is_enable_seq_parallel():
            return dist.get_rank(ParallelManager.engine.seq_parallel_group)
        # If sequence parallelism is not enabled, the process is effectively rank 0
        # within a group of size 1.
        return 0
