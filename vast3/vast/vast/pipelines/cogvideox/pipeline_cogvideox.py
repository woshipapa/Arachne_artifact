...
class CogVideoXPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if cn_images is not None:
            ...
            cn_images = cn_images / 255.0
            cn_images = self.normalize(cn_images)
            cn_images = rearrange(cn_images.unsqueeze(0), "b f c h w -> b c f h w")
            cn_images = self.vae.encode(
                cn_images.to(dtype=self.vae.dtype, device=self.vae.device)
            ).latent_dist.sample()
            if getattr(self, "guider", None) is not None:
                ...
                cn_latents = self.guider(
                    cn_images.to(dtype=self.guider.dtype, device=self.guider.device)
                )
            else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if cn_images is not None:
                    latent_model_input = torch.cat(
                        [latent_model_input, cn_latents], dim=2
                    )
