...
class Trainer:
        ...
        if skip_batches > 0:
            for i in range(len(self._train_dataloaders)):
                ...
                if actual_skips < skip_batches:
                    self.logger.warning(
                        f"skip_batches {skip_batches} 超过数据加载器长度 {max_skips}，实际跳过 {actual_skips} 个批次。"
                    )
    ...
    def timeit_context(self, name):
        ...
        return TimeIt(self, name)  # 只需传入 name 参数

    # data_config is dataloader.train
    def get_dataloader(self, data_config):
        ...
        # ClipDataset list  with  after filter
        dataset = build_dataset(data_config.dataset)
        # print(f'dataset = {dataset.data_list}')
        filter_cfg = data_config.get("filter", None)  #None
        # print(f'{filter_cfg}')
        if filter_cfg is not None:
            # print(f'---------------------before dataset.filter()-----------------------  {filter_cfg}')
            dataset.filter(**filter_cfg)
            # print(f'---------------------after dataset.filter()-----------------------')
    ...
    def _setup_logger(self):
        """为每个rank设置独立的logger。"""
        if self.rank_logger:  # 防止重复设置
            return

        # 确保日志目录存在
        os.makedirs(self.log_dir, exist_ok=True)

        self.rank_logger = logging.getLogger(f"{self.__class__.__name__}_rank_{self.rank}")
        self.rank_logger.setLevel(logging.INFO)

        # ★★★★★★★★★★★★★★★★★★★★
        # ★ 这里是核心改动 ★
        # ★★★★★★★★★★★★★★★★★★★★
        # 新的格式化字符串，增加了 %(funcName)s 和 %(lineno)d
        ...
        # 配置 StreamHandler
        if not any(isinstance(h, logging.StreamHandler) for h in self.rank_logger.handlers):
            stream_handler = logging.StreamHandler()
            stream_handler.setFormatter(formatter)
            self.rank_logger.addHandler(stream_handler)

        # 配置 FileHandler
        log_file = os.path.join(self.log_dir, f"rank_{self.rank}.log")
        if not any(
            isinstance(h, logging.FileHandler) and h.baseFilename == log_file
            for h in self.rank_logger.handlers
        ):
            file_handler = logging.FileHandler(log_file, mode="a")
            file_handler.setFormatter(formatter)
            self.rank_logger.addHandler(file_handler)

        self.rank_logger.propagate = False

    def prepare_after_dist_initialized(self):
            self.rank = dist.get_rank()
            self.world_size = dist.get_world_size()
            self._setup_logger()
            self.rank_logger.info(
                "--- logger is being initialized  ---"
            )

    def train(self):
        # from utils import GPU_Memory_Tracker
        # tracker = GPU_Memory_Tracker(prefix='whole',interval=0.05,use_distributed=True)
        from torch.utils.tensorboard import SummaryWriter

        # 创建 SummaryWriter 对象
        # writer = SummaryWriter(log_dir='./log_45_dp8')
        ...
        batch_dict = None # 初始化 batch_dict 为 None
        # def trace_handler(prof):

        #     prof.export_chrome_trace(f"/root/deepspeed_baseline/new_profiler/rank_{dist.get_rank()}.json")
        #     rank = dist.get_rank() if dist.is_initialized() else 0
        #     rank_dir = f"/root/deepspeed_baseline/new_profiler/prof_rank_{rank}"
        #     os.makedirs(rank_dir, exist_ok=True)

        #         # 2. profile summary (CPU & CUDA)
        #     with open(os.path.join(rank_dir, "summary.txt"), "w") as f:
        ...
        #     # 3. stack 信息（可选）
        #     prof.export_stacks(os.path.join(rank_dir, "stacks.txt"),
        ...
        while self._cur_step < self.max_steps:
            ...
            if (self._cur_step - 1) % 4 == 0:
                data_start_tic = time.time()
                try:
                    batch_dict = next(dataloader_iter)
                except StopIteration:
                    # 如果 dataloader 数据已耗尽，则重新创建迭代器
                    self.accelerator.print("Dataloader iterator has been exhausted. Re-initializing.")
                    dataloader_iter = iter(self.train_dataloader)
                    batch_dict = next(dataloader_iter)
            ...
            # global_timer.start(f"vae_forward_bs_{batch_size}_f_{num_frames}_h_{height}_w_{width}")
            # latents = self.forward_vae(images) * self.vae.config.scaling_factor
            # global_timer.stop(f"vae_forward_bs_{batch_size}_f_{num_frames}_h_{height}_w_{width}")
            # batch_dict['latents'] = latents
            # 在每个 rank 上打印 image tensor 的形状
            # 使用 accelerator.print 可以在主进程上统一打印，避免多进程打印混乱
            # 如果想每个进程都独立打印，可以直接用 print()
            bs , frame, _ , h, w = batch_dict['images'].shape
            self.rank_logger.info(f"[Rank {torch.distributed.get_rank()}] [Iter {(self._cur_step - 1) // 4}] Fetched new data. Image batch shape: {batch_dict['images'].shape}")
            self.model.train()
            # prof.start()
            for _ in range(self.gradient_accumulation_steps):
                ...
                # data_start_tic = time.time()
                # batch_dict = next(dataloader_iter)
                # print(f"[Rank {torch.distributed.get_rank()}] batch_dict = {batch_dict}")
                # self._data_time = time.time() - data_start_tic
                with self.accelerator.accumulate(*self.models):
                    global_timer.start(f"cur_step{self._cur_step}")
                    global_timer.start(f"forward_step_{bs}_{frame}_{h}_{w}_sp4")
                    losses = self.forward_step(batch_dict)
                    global_timer.stop(f"forward_step_{bs}_{frame}_{h}_{w}_sp4")
                    loss = self.parse_losses(losses)
                    global_timer.start(f"backward_step_{bs}_{frame}_{h}_{w}_sp4")
                    self.backward_step(loss)
                    global_timer.stop(f"backward_step_{bs}_{frame}_{h}_{w}_sp4")
                    global_timer.stop(f"cur_step{self._cur_step}")
                    # writer.add_scalar('Loss/train', loss.item(), self._cur_step)
            # prof.stop()
            # if dist.get_rank() == 0:
            #     tracker.stop_monitoring()  
            if hasattr(global_timer, "step"):
                global_timer.step()
            self.print_step()
            self.eval()
    ...
    def print_step(self):
        ...
        if self._by_epoch and self.cur_step % self.epoch_size == 0:
            ...
            self.rank_logger.info("Total_time: %s" % time_cost)
    ...
    def parse_losses(self, losses):
        ...
        if isinstance(losses, dict):
            ...
            # print(f'[Rank {dist.get_rank()}] loss = {loss}, len+total_loss = {len(total_loss)}_{total_loss}')
