        ...
        **kwargs,
        ...
        if executable is None:
            process = subprocess.run(
                ["which", "accelerate"], env=env, capture_output=True, text=True
            )
        ...
        self.num_processes = (int(os.environ.get("WORLD_SIZE", 1)) * len(gpu_ids)) 
        self.main_process_ip = main_process_ip
        self.main_process_port = main_process_port

    def launch(self, script):
        command = "{} launch".format(self.executable)
        command += " --config_file {}".format(self.config_file)
        command += " --num_cpu_threads_per_process {}".format(
            self.num_cpu_threads_per_process
        )

        command += " --num_machines {}".format(
            os.environ.get("WORLD_SIZE", 1)
        )  # 获取机器数量，默认值为 1
        command += " --machine_rank {}".format(
            os.environ.get("RANK", 0)
        )  # 当前机器的编号，默认值为 0
        command += " --main_process_ip {}".format(
            os.environ.get("MASTER_ADDR", self.main_process_ip)
        )  # 主节点 IP 地址，默认值为本地地址
        command += " --main_process_port {}".format(
            os.environ.get("MASTER_PORT", self.main_process_port)
        )  # 主节点的通信端口，默认值为 29500

        command += " --num_processes {}".format(self.num_processes)  # 添加总进程数到命令行
        ...
        try:
            # while True:
                ...
                print("\ncomplete process. process.returncode=", process.returncode)
...
def launch_from_config(config_path, runners=None, gpu_memory=None, seconds=10):
    ...
    if runners is None:
        try:
            ...
            print(f"can not found runner in config {config_path}")
            exit()
    ...
    if num_machines == 1 and len(gpu_ids) == 1:
        ...
        run_tasks(config_path, runners)
    else:
        runners = ",".join(runners)
        launcher = Launcher(**config.launch)
        file_path = os.path.join(
            os.path.abspath(__file__).split("launch")[0], "run_task.py"
        )
        launcher.launch(
            "{} --config {} --runners {}".format(file_path, config_path, runners)
        )
