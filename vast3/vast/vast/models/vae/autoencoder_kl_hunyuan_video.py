...
class AutoencoderKLHunyuanVideo(ModelMixin, ConfigMixin):
    ...
    def move_all_tiles_to_local_device(self,gathered_tiles, device):
        """
        将 all_gather_object 得到的 tile 字典列表中的所有张量移动到当前设备上。

        Args:
            gathered_tiles (List[Dict[Tuple[int,int], Tensor]]): 每个 rank 的 tile 编码结果
            device (torch.device): 当前 rank 的本地 CUDA 设备
        Returns:
            Dict[Tuple[int,int], Tensor]: 所有 tile，统一在本地 device 上
        """
        all_tiles = {}
        for tile_dict in gathered_tiles:
            for coord, tile in tile_dict.items():
                tile_on_device = tile.to(device)
                all_tiles[coord] = tile_on_device
        return all_tiles
    def tiled_encode(self, x: torch.Tensor) -> AutoencoderKLOutput:
        ...
        for i, j in all_tile_coords:
            ...
            tiles_by_shape[shape].append((i, j))

        # 3. 将每个 shape 组内的 tile 分配给不同 rank
        local_tile_indices = []
        for shape, coords_list in sorted(tiles_by_shape.items()):
            local_tiles_for_this_shape = coords_list[rank::world_size]
            local_tile_indices.extend(local_tiles_for_this_shape)
        ...
        # 【新增日志】统计并打印每个 rank 持有的不同 shape 的 tile 数量
        # 为了避免重复计算，我们再次遍历 local_tile_indices 并计算形状
        local_shape_counts = Counter()
        for i, j in local_tile_indices:
            actual_h = min(i + self.tile_sample_min_height, height) - i
            actual_w = min(j + self.tile_sample_min_width, width) - j
            shape = (actual_h, actual_w)
        ...
        # 格式化打印日志
        # print(f"--- [Rank {rank}] ---")
        # print(f"Total tiles assigned: {len(local_tile_indices)} out of {total_tiles}")
        # print("Shape distribution:")
        # if not local_shape_counts:
        #     print("  - No tiles assigned.")
        # else:
        #     for shape, count in sorted(local_shape_counts.items()):
        #         print(f"  - Shape {shape}: {count} tiles")
        # print("--------------------", flush=True) # flush=True 确保并行打印时信息能立刻显示
        from my_utils import global_timer
        # Encode local tiles
        ...
        for i, j in local_tile_indices:
            ...
            if os.environ.get("VAE_PROFILE_BATCH") == "1":
                global_timer.start(tile_str)
            tile = self.encoder(tile)
            tile = self.quant_conv(tile)
            if os.environ.get("VAE_PROFILE_BATCH") == "1":
                global_timer.stop(tile_str)
            local_encoded_tiles[(i, j)] = tile

        # Gather all tiles to all ranks (高性能 tensor all_gather 路径)
        all_tiles = gather_tiled_tensors_with_all_gather(
            local_tiles=local_encoded_tiles,
            group=mpu.get_context_parallel_group(),
            device=device,
            fallback_dtype=x.dtype,
        )

        # Stitch tiles together (这部分完全不变)
        row_positions = sorted(set(i for i, _ in all_tile_coords))
        col_positions = sorted(set(j for _, j in all_tile_coords))
        ...
        for i in row_positions:
            ...
            for j in col_positions:
                tile = all_tiles[(i, j)]
                if i > row_positions[0]:
                    tile = self.blend_v(all_tiles[(i - self.tile_sample_stride_height, j)], tile, blend_height)
                if j > col_positions[0]:
                    tile = self.blend_h(all_tiles[(i, j - self.tile_sample_stride_width)], tile, blend_width)
                tile = tile[:, :, :, :tile_latent_stride_height, :tile_latent_stride_width]
                row.append(tile)
            rows.append(torch.cat(row, dim=4))

        enc = torch.cat(rows, dim=3)[:, :, :, :latent_height, :latent_width]
        # torch.save(enc,"dist_vae.pt")
        # global_timer.stop("sort+blend")
        ...
        # else:
        # rows = []
        # global_timer.start("all_tile_encode")
        # for i in range(0, height, self.tile_sample_stride_height):
        #     row = []
        #     for j in range(0, width, self.tile_sample_stride_width):
        #         tile = x[:, :, :, i : i + self.tile_sample_min_height, j : j + self.tile_sample_min_width]
        #         # if rank == 0 and i == 0 and j == 0:
        #         #     torch.save(tile,"all_tile_00_input.pt")
        #         # if rank == 3 and i == 0 and j == 0:
        #         #     torch.save(tile,"all_tile_00_input_3.pt")        
        #         # print(tile.shape)
        #         # global_timer.start(f"h_[{i,i + self.tile_sample_min_height-1}]_w_[{j,j + self.tile_sample_min_width-1}]_shape_")
        #         tile = self.encoder(tile)
        #         tile = self.quant_conv(tile)
        #         # if rank == 0 and i == 0 and j == 0:
        #         #     torch.save(tile,"all_tile_00_encoder.pt")
        #         # if rank == 3 and i == 0 and j == 0:
        #         #     torch.save(tile,"all_tile_00_encoder_3.pt")    
        #         # global_timer.stop(f"h_[{i,i + self.tile_sample_min_height-1}]_w_[{j,j + self.tile_sample_min_width-1}]_shape_")
        #         row.append(tile)
        #     rows.append(row)
        # global_timer.stop("all_tile_encode")
        # # torch.save(rows, f"rows_{rank}.pt")
        # result_rows = []
        ...
        # torch.save(enc,"sequencial_vae.pt")
        # exit(1)
        return enc
    ...
    def _temporal_tiled_encode(self, x: torch.Tensor) -> AutoencoderKLOutput:
        ...
        row = []
        from my_utils import global_timer
        # global_timer.start("temporal_tiled_encode")
        for i in range(0, num_frames, self.tile_sample_stride_num_frames):
            ...
            if i > 0:
                tile = tile[:, :, 1:, :, :]
            row.append(tile)
        # global_timer.stop("temporal_tiled_encode")
        result_row = []
