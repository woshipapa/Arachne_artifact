...
def get_model_path(model_name_or_path):
    ...
    # When weights are unavailable, fall back to default (random) init instead of
    # failing -- mirrors dynamic-flex building the model from config without a
    # checkpoint. Gated on ALLOW_MISSING_WEIGHTS so normal runs still error out.
    if os.environ.get("ALLOW_MISSING_WEIGHTS") == "1":
        print(f"[get_model_path] '{model_name_or_path}' not found; "
              f"ALLOW_MISSING_WEIGHTS=1 -> default (random) init.")
        return model_name_or_path
