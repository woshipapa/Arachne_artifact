...
#     debugger = PrecisionDebugger(config_path="/root/vast2/debug_logs/msprobe_config.json")
#     def dummy_torchscript(f):
#         return f 
#     torch.jit.script = dummy_torchscript
# else:
#     print("debug is off")
...
class HunyuanVideoAttnProcessor2_0:
    ...
    def __call__(
        ...
        # 1. QKV projections
        # if dist.get_rank() == 0:
            # print(f'...................enter to_qkv......................   allo_mm : {torch.cuda.memory_allocated() / 1024**3}G')    
       ...
       # 这里是如果是sp的话，这个inner_dim 就是 dim / N
        ...
        # if dist.get_rank() == 0:
        #     print(f'...................enter image_rotary_emb......................')    
        ...
        if encoder_hidden_states is not None:
            ...
            # print(f'getattr:{getattr(attn, "to_out", None)}')
            if getattr(attn, "to_out", None) is not None:
                # print(f'to_out:{attn.to_out}')
                hidden_states = attn.to_out[0](hidden_states)
                hidden_states = attn.to_out[1](hidden_states)

            if getattr(attn, "to_add_out", None) is not None:
                # print(f'to_add_out:{attn.to_add_out}')
                encoder_hidden_states = attn.to_add_out(encoder_hidden_states)

        # if dist.get_rank() == 0:
            # print(f'attnProcessor finishing.... allo_mm : {torch.cuda.memory_allocated() / 1024**3}G')
...
class HunyuanVideoSingleTransformerBlock(nn.Module):
    ...
    #         def begin_hook(module, grad_input):
    #             # if os.environ.get('DEBUG') == "1":
    #             #     debugger.start()
    #             if torch.distributed.get_rank() == 0:
    #                 logger.info(f"{module_name}, {module.__class__}, begin backward")
    #             return
    #         def end_hook(module, grad_input, grad_output):
    #             # if os.environ.get('DEBUG') == "1":
    #             #     debugger.start()
    #             if torch.distributed.get_rank() == 0:
    #                 logger.info(f"{module_name}, {module.__class__}, end backward")
    #             return
    #         return begin_hook, end_hook     
        
    #     for name, m in self.named_modules():
    #         # print(f'name: {name}')
    #         begin_hook, end_hook = get_hook(name)
    ...
    def forward(
        ...
        # if torch.distributed.get_rank() == 0:
            # logger.info(f'allocated_mm = {torch.cuda.memory_allocated() / 1024**3}G, reserved_mm = {torch.cuda.memory_reserved() / 1024**3}G')
        ...
        # logger.info(f'[Single Block] after norm  hidden_states shape(only split hidden_states ) is {norm_hidden_states.shape} , norm_encoder shape is {norm_encoder_hidden_states.shape},gate shape is {gate.shape}, mlp_output shape is {mlp_hidden_states.shape}')
        # 2. Attention
...
class HunyuanVideoTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(
        ...
        # if dist.get_rank() == 0:
        #     Debugger.register_debug_hook(self.single_transformer_blocks,tag='whole')
        #     register_debug_hook(self.single_transformer_blocks[39].norm,tag='norm')
        # 5. Output projection
    ...
    #         param.register_hook(build_hook(name))
    #         # logger.info(f'name: {name},hooks :{param._backward_hooks}')
    ...
    def forward(
        ...
        from accelerate.sp_utils import ProfilerWrapper
        ...
        profiler = ProfilerWrapper(is_st=False,enable_record_cuda_mm=True,enable_print_summary=False)
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            ...
            for i, block in enumerate(self.transformer_blocks):
                    # with profiler:
                        if dist.get_rank() == 0:
                            logger.info(f'..............enter the {i+1} dual-stream block............., allocated_mm = {torch.cuda.memory_allocated() / 1000**3}, reserved_mm = {torch.cuda.memory_reserved() / 1000**3}')    
            ...
            for i, block in enumerate(self.single_transformer_blocks):
                # with profiler:    
                    if dist.get_rank() == 0:
                        logger.info(f'..............enter the {i+1} single-stream block............., allocated_mm = {torch.cuda.memory_allocated() / 1000**3}, reserved_mm = {torch.cuda.memory_reserved() / 1000**3}')    
                        logger.info(f'hidden_states = {hidden_states.shape}')
                    hidden_states, encoder_hidden_states = (
                        torch.utils.checkpoint.checkpoint(
                            create_custom_forward(block),
