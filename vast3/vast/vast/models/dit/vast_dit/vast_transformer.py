...
class VASTPatchEmbed(nn.Module):
    def __init__(
        self, 
        patch_size: List[Tuple] = [(1,2,2),(2,4,4)],          #patch_size=[(p_t,p_h,p_w),..]
        in_channels: int = 16,
        ...
        for p_t,p_h,p_w in self.patch_size:
            if p_t//1==1 and p_h//2==1 and p_w//2==1:
                patch_layers.append(nn.Conv3d(in_channels, embed_dim, kernel_size=(1,2,2), stride=(1,2,2), bias=bias))
            elif p_t//1==1 and p_h//2==2 and p_w//2==2:
                patch_layers.append(nn.Conv3d(in_channels, embed_dim, kernel_size=(1,2,2), stride=(1,2,2), bias=bias))
            else:
                assert False
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        for i, (p_t, p_h, p_w) in enumerate(self.patch_size):
            ...
            if p_t//1==1 and p_h//2==1 and p_w//2==1:
                ...
                image_embed_p = F.interpolate(image_embed_p, scale_factor=(1,0.5,0.5), mode='trilinear')
                # import pdb; pdb.set_trace()
            else:
                assert False
...
class VASTTransformerModel(ModelMixin, ConfigMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        if kwargs.pop("from_giga", False):
            from vastbase.models.dit import CogVideoXTransformer3DModel
            ...
            if patch_size is not None:
                model = VASTTransformerModel(patch_size=patch_size)
            else:
                model = VASTTransformerModel()
    ...
    def forward(
        ...
        inter_size = []
        for p_t, p_h, p_w in self.patch_embed.patch_size:
            inter_size.append((num_frames//p_t, height//p_h, width//p_w))
        ...
        # image_rotary_emb_cgvx = torch.load('image_rotary_emb.pth')
        # max_error = torch.max(torch.abs(image_rotary_emb_cgvx[0] - image_rotary_emb[0]))
        # max_error = max_error + torch.max(torch.abs(image_rotary_emb_cgvx[1] - image_rotary_emb[1]))
        # print("Max Error:", max_error)
        # max_error = torch.max(torch.abs(image_rotary_emb_cgvx[0] - image_rotary_emb_hr[0]))
        # max_error = max_error + torch.max(torch.abs(image_rotary_emb_cgvx[1] - image_rotary_emb_hr[1]))
        # print("Max Error:", max_error)
        # import pdb; pdb.set_trace()
        # image_rotary_emb_hr = image_rotary_emb_cgvx
        # image_rotary_emb = image_rotary_emb_cgvx
        ...
        hidden_states_ms = hidden_states.transpose(1, 2).unflatten(2,(inter_size[-1][0], inter_size[-1][1], inter_size[-1][2]))# BCFHW <- BNC
        hidden_states_ms = F.interpolate(hidden_states_ms, size=(inter_size[0][0], inter_size[0][1], inter_size[0][2]), mode='trilinear')
        hidden_states_ms = hidden_states_ms.flatten(2).transpose(1,2)# BCFHW -> BNC
        hidden_states_ms = hidden_states_hr + hidden_states_ms
        ...
        hidden_states_lr = hidden_states_lr.transpose(1, 2).unflatten(2,(inter_size[-1][0], inter_size[-1][1], inter_size[-1][2]))# BCFHW <- BNC
        hidden_states_lr = F.interpolate(hidden_states_lr, size=(inter_size[0][0], inter_size[0][1], inter_size[0][2]), mode='trilinear')
        hidden_states_lr = hidden_states_lr.flatten(2).transpose(1,2)# BCFHW -> BNC
        hidden_states_ms = hidden_states_ms + hidden_states_lr
        ...
        output = hidden_states.reshape(batch_size, num_frames//p[0], height//p[1], width//p[2], -1, p[0], p[1], p[2])
        output = output.permute(0, 1, 5, 4, 2, 6, 3, 7).flatten(6, 7).flatten(4, 5).flatten(1, 2)
        # output = output.permute(0, 4, 1, 5,  2, 6, 3, 7).flatten(6, 7).flatten(4, 5).flatten(2, 3)
