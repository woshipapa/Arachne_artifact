...
def _all_in_all_with_text(hidden_states, text_seq_length, sp_group, sp_size, mode):
    if mode == 1:
        hidden_states = all_to_all(
            hidden_states, scatter_dim=1, gather_dim=2, group=sp_group
        )
        ...
        encoder_hidden_states = rearrange(
            encoder_hidden_states, "b h p s c -> b h (p s) c"
        )
        ...
        encoder_hidden_states = rearrange(
            encoder_hidden_states, "b h (p s) c -> b h p s c", p=sp_size
        )
        hidden_states = rearrange(hidden_states, "b h (p s) c -> b h p s c", p=sp_size)
        hidden_states = torch.cat([encoder_hidden_states, hidden_states], dim=3)
        hidden_states = rearrange(hidden_states, "b h p s c -> b h (p s) c")
        hidden_states = all_to_all(
            hidden_states, scatter_dim=2, gather_dim=1, group=sp_group
        )
    else:
        assert False
...
class CogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        if parallel_manager.is_enable_seq_parallel():
            ...
            # print(f"before alltoall images_query shape is {images_query.shape}, txt_query shape is {txt_query.shape}")
            ...
            txt_query,txt_key,txt_value = map(
                lambda x: split_sequence(x, seq_parallel_group, dim = 1),
                [txt_query,txt_key,txt_value]
            )
            # print(f"after alltoall images_query shape is {images_query.shape}, txt_query shape is {txt_query.shape}")
            # 现在的images qkv都是在seq_length都是带有padding的
            # 这里需要remove padding
            images_query,images_key,images_value = map(
                lambda x: remove_padding(x, seq_parallel_group, padding_key="hidden_states_pad", dim=2),
                [images_query,images_key,images_value]
            )
            # images_query, images_key, images_value = map(
        ...
        #     query = _all_in_all_with_text(
        #         query, text_seq_length, sp_group, sp_size, mode=1
        #     )
        #     key = _all_in_all_with_text(key, text_seq_length, sp_group, sp_size, mode=1)
        #     value = _all_in_all_with_text(
        #         value, text_seq_length, sp_group, sp_size, mode=1
        #     )
        ...
        if parallel_manager.is_enable_seq_parallel():
            hidden_states = add_padding(hidden_states,seq_parallel_group,padding_key ="hidden_states_pad",dim=1)
            hidden_states = all_to_all_comm(hidden_states, seq_parallel_group, scatter_dim=1, gather_dim=2)
            encoder_hidden_states = gather_sequence(encoder_hidden_states, seq_parallel_group, dim=2) # b txt_seq n d
        # print(f"after output alltoall")
...
class CogVideoXTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if parallel_manager.is_enable_seq_parallel():
            ...
            hidden_states_pad_key = "hidden_states_pad"
            hidden_states_seq_dim = 1
            set_pad(hidden_states_pad_key, hidden_states.shape[hidden_states_seq_dim], seq_parallel_group)
            hidden_states = split_sequence(
                hidden_states,
                seq_parallel_group,
                dim=hidden_states_seq_dim,
                pad=get_pad(hidden_states_pad_key)
            )
        ...
        if parallel_manager.is_enable_seq_parallel():
            hidden_states = gather_sequence(
                ...
                pad=get_pad("hidden_states_pad")
            )
            hidden_states = remove_padding(hidden_states, seq_parallel_group, padding_key = "hidden_states_pad", dim = 1)
