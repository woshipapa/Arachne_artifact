...
def get_rot_mat_t(angle3d, rot_axis):
    ...
    if isinstance(angle3d, torch.Tensor):
        if rot_axis in (0, 1, 2):
            ...
            rot_mat_t = _get_rot_mat_t(
                torch.sin(angle3d), torch.cos(angle3d), ones, zeros, rot_axis
            )
            rot_mat_t = (
                torch.cat(rot_mat_t, dim=0)
                .reshape((3, 3, -1))
                .permute(2, 0, 1)
                .contiguous()
            )
            ...
            rot_mat_t_x = _get_rot_mat_t(
                torch.sin(angle3d[:, 0]), torch.cos(angle3d[:, 0]), ones, zeros, 0
            )
            rot_mat_t_y = _get_rot_mat_t(
                torch.sin(angle3d[:, 1]), torch.cos(angle3d[:, 1]), ones, zeros, 1
            )
            rot_mat_t_z = _get_rot_mat_t(
                torch.sin(angle3d[:, 2]), torch.cos(angle3d[:, 2]), ones, zeros, 2
            )
            ...
            rot_mat_t = torch.einsum(
                "ija,jka,kha->aih", rot_mat_t_z, rot_mat_t_y, rot_mat_t_x
            )
        else:
            assert False
    else:
        ...
        if rot_axis in (0, 1, 2):
            ...
            rot_mat_t = _get_rot_mat_t(
                np.sin(angle3d), np.cos(angle3d), ones, zeros, rot_axis
            )
            rot_mat_t = (
                np.concatenate(rot_mat_t, axis=0).reshape((3, 3, -1)).transpose(2, 0, 1)
            )
        elif rot_axis == -1:
            ...
            rot_mat_t_x = _get_rot_mat_t(
                np.sin(angle3d[:, 0]), np.cos(angle3d[:, 0]), ones, zeros, 0
            )
            rot_mat_t_y = _get_rot_mat_t(
                np.sin(angle3d[:, 1]), np.cos(angle3d[:, 1]), ones, zeros, 1
            )
            rot_mat_t_z = _get_rot_mat_t(
                np.sin(angle3d[:, 2]), np.cos(angle3d[:, 2]), ones, zeros, 2
            )
...
def points3d_to_points2d(points3d, cam_intrinsic):
    ...
    if isinstance(points3d, torch.Tensor):
        if cam_intrinsic.shape[1] == 4:
            points3d = torch.cat(
                [points3d, points3d.new_ones((points3d.shape[0], 1))], dim=-1
            )  # (N, 4)
...
def depth_to_points3d(depth, cam_k=None, cam_scale=1.0, method=2):
    ...
    if cam_k is None:
        ...
        points_y = (ymap - cy) / fy
        points3d = np.stack(
            [points_x * points_z, points_y * points_z, points_z], axis=-1
        ).astype(np.float32)
    elif method == 2:
        ...
        points3d = np.stack(
            [xmap * points_z, ymap * points_z, points_z], axis=-1
        ).astype(np.float32)
...
def points3d_in_surfaces3d(points3d, surfaces3d):
    ...
    if isinstance(points3d, torch.Tensor):
        ...
        normal_vec = torch.cross(
            surface_vec[:, :, 0, :], surface_vec[:, :, 1, :], dim=-1
        )  # (M, 6, 3)
