...
def dims_to_norm_corners(dims, origin=0.5):
    ...
    norm_corners -= origin
    norm_corners = dims.reshape((-1, 1, 2)) * norm_corners.reshape(
        (1, 4, 2)
    )  # (N, 4, 2)
    return norm_corners
...
def check_boxes(boxes, image_shape, offset=0):
    ...
    if isinstance(boxes, torch.Tensor):
        ...
        flag = torch.logical_and(
            boxes[:, 0] < boxes[:, 2] + offset, boxes[:, 1] < boxes[:, 3] + offset
        )
        assert flag.all()
        flag = torch.logical_and(
            boxes[:, 0] <= image_w - 1, boxes[:, 2] <= image_w - offset
        )
        assert flag.all()
        flag = torch.logical_and(
            boxes[:, 1] <= image_h - 1, boxes[:, 3] <= image_h - offset
        )
        assert flag.all()
    else:
...
def rotate_boxes(boxes, rot_mat_t, offset=0):
    ...
    corners = points_utils.rotate_points(corners.reshape(-1, 2), rot_mat_t).reshape(
        (-1, 4, 2)
    )
    boxes = corners_to_boxes(corners)
    return boxes
    ...
    if isinstance(boxes, torch.Tensor):
        ...
        if keep_outside_center:
            ...
            keep = torch.logical_and(
                crop_range[:2] <= centers, centers <= crop_range[2:]
            ).all(axis=1)
...
def points_in_boxes(points, boxes):
    ...
    for i in range(points.shape[0]):
        for j in range(boxes.shape[0]):
            if (
                boxes[j, 0] <= points[i, 0] <= boxes[j, 2]
                and boxes[j, 1] <= points[i, 1] <= boxes[j, 3]
            ):
                indices[i, j] = 1
    return indices
    ...
    if is_aligned:
        ...
        if mode == "giou":
            if isinstance(boxes_a, torch.Tensor):
                ...
                enclosed_wh = (enclosed_rb - enclosed_lt + offset).clamp(
                    min=0
                )  # (n, 2)
            else:
                enclosed_lt = np.minimum(boxes_a[:, :2], boxes_b[:, :2])  # (n, 2)
                enclosed_rb = np.maximum(boxes_a[:, 2:4], boxes_b[:, 2:4])  # (n, 2)
                enclosed_wh = np.maximum(
                    enclosed_rb - enclosed_lt + offset, 0
                )  # (n, 2)
        ...
        if mode == "giou":
            if isinstance(boxes_a, torch.Tensor):
                ...
                enclosed_rb = torch.max(
                    boxes_a[:, None, 2:4], boxes_b[None, :, 2:4]
                )  # (n, m, 2)
