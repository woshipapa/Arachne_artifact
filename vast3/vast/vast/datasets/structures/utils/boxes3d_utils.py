...
def dims3d_to_norm_corners3d(dims3d, origin=(0.5, 0.5, 0.5)):
    ...
    norm_corners3d = dims3d.reshape((-1, 1, 3)) * norm_corners3d.reshape(
        (1, 8, 3)
    )  # (N, 8, 3)
...
def corners2d_to_corners3d(corners, depths, cam_intrinsic):
    ...
    if isinstance(corners, torch.Tensor):
        ...
        if cam_intrinsic.shape[1] == 4:
            ones = corners3d.new_ones(corners_shape, dtype=corners3d.dtype)
            corners3d = torch.cat([corners3d, ones], dim=-1)  # (N, 8, 4)
        corners3d = torch.matmul(
            corners3d, torch.inverse(cam_intrinsic).t()
        )  # (N, 8, 4)
    else:
...
def bev_boxes2d_to_nearest_bev_boxes2d(bev_boxes):
    ...
    normed_rotations = limit_period(
        bev_boxes[:, 4], 0.5, np.pi
    )  # limit rad to [-pi / 2, pi / 2]
    if isinstance(bev_boxes, torch.Tensor):
        ...
        centers = bev_boxes[:, :2]
        nearest_bev_boxes = np.concatenate(
            [centers - dims / 2, centers + dims / 2], axis=-1
        )
...
def overlaps(
    ...
    if method.startswith("minmax"):
        ...
        if isinstance(boxes3d_a, torch.Tensor):
            ...
            rb = torch.min(
                boxes3d_a[:, None, 3:6], boxes3d_b[None, :, 3:6]
            )  # (n, m, 3)
        ...
        if method == "depth_nearest_bev":
            ...
            if isinstance(boxes3d_a, torch.Tensor):
                lt = torch.max(
                    boxes_a_bev[:, None, :2], boxes_b_bev[None, :, :2]
                )  # (n, m, 2)
                rb = torch.min(
                    boxes_a_bev[:, None, 2:4], boxes_b_bev[None, :, 2:4]
                )  # (n, m, 2)
