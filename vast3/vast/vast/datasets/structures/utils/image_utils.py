...
def get_rot_mat(angle, src_shape, dst_shape):
    ...
    if isinstance(angle, torch.Tensor):
        ...
        t_mat = torch.tensor(
            [1, 0, -src_w / 2.0, 0, 1, -src_h / 2.0, 0, 0, 1], dtype=angle.dtype
        ).reshape((3, 3))
        r_mat = torch.cat(
        ...
        z_mat = torch.tensor(
            [1, 0, dst_w / 2.0, 0, 1, dst_h / 2.0, 0, 0, 1], dtype=angle.dtype
        ).reshape((3, 3))
...
def pad_image(image, dst_size, mode="top_left", pad_value=0):
    ...
    if image.ndim == 3:
        ...
        new_image = np.full(
            (dst_height, dst_width), fill_value=pad_value, dtype=image.dtype
        )
    else:
        assert False
    if mode == "top_left":
