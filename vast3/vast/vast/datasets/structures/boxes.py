...
class Boxes(BaseStructure):
    ...
    def flip(self, image_shape, direction="horizontal"):
        self.tensor = boxes_utils.flip_boxes(
            self.tensor, image_shape, direction, self.offset
        )
    ...
    def crop(self, crop_range, keep_outside_center=False, keep_outside_boxes=False):
        self.tensor, keep = boxes_utils.crop_boxes(
            crop_range, keep_outside_center, keep_outside_boxes
        )
        return keep

    def scale(self, scale_hw, image_shape):
        self.tensor = boxes_utils.scale_boxes(
            self.tensor, scale_hw, image_shape, self.offset
        )

    def overlaps(self, boxes, mode="iou", is_aligned=False, offset=0, eps=1e-6):
        if not isinstance(boxes, torch.Tensor):
            boxes = self.tensor.new_tensor(boxes)
        return boxes_utils.overlaps(
            self.tensor, boxes, mode=mode, is_aligned=is_aligned, offset=offset, eps=eps
        )
