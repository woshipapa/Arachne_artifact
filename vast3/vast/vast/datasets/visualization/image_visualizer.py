...
class ImageVisualizer:
    ...
    def resize(self, val, mode, interp=cv2.INTER_LINEAR):
        ...
        self.image = cv2.resize(
            self.image, None, None, fx=scale[1], fy=scale[0], interpolation=interp
        )
        return self
    ...
    def draw_text(self, text, mode="top", char_height=40, char_width=18):
        if mode == "top":
            ...
            self.image = image_utils.concat_images(
                [image, self.image], direction="vertical"
            )
        else:
            assert False
    ...
    def draw_boxes(
        self, boxes, classes=None, color=None, thickness=2, texts=None, show_num=False
    ):
        # boxes: (num_boxes, 4)
        if len(boxes) == 0:
            return
    ...
    def draw_corners(
        ...
        for i, corner in enumerate(corners):
            ...
            if show_ori:
                ...
                ori_bottom_start = np.mean(
                    corner[bottom_indexes], axis=0, dtype=np.int32
                )
                ori_bottom_end = np.mean(
                    corner[bottom_indexes[-2:]], axis=0, dtype=np.int32
                )
                cv2.line(
                    self.image,
            ...
            if show_num:
                for k in range(8):
                    text = "%d" % k
                    org = self._get_point_text_org(
                        text, (corner[k, 0], corner[k, 1]), offset=thickness
                    )
                    cv2.putText(
                        self.image,
                        text=text,
            ...
            for j in range(keypoints.shape[1]):
                ...
                if v > 0:
                    ...
                    if show_num:
                        text = "%d" % j
                        org = self._get_point_text_org(
                            text, (x, y), offset=radius + thickness
                        )
                        cv2.putText(
                            self.image,
                            text=text,
    ...
    def draw_lanes(self, lanes, lane_labels=None, thickness=2):
        for i, lane in enumerate(lanes):
            ...
            color = self.colormap[color]
            cv2.polylines(
                self.image, [lane], isClosed=False, color=color, thickness=thickness
            )
    ...
    def draw_points3d(self, points3d, lidar2cam, cam_intrinsic, **kwargs):
        ...
        points3d_homo = np.concatenate(
            [points3d[:, :3], np.ones([points3d.shape[0], 1])], axis=1
        )
    ...
    def draw_boxes3d_camera(self, boxes3d_camera, classes, cam_intrinsic, **kwargs):
        if len(boxes3d_camera) == 0:
            return
        corners3d_camera = boxes3d_utils.boxes3d_to_corners3d(
            boxes3d_camera, rot_axis=1
        )
        keep = corners3d_camera[..., 2].mean(-1) > 0
        corners3d_camera = boxes3d_utils.crop_corners3d(corners3d_camera)
        corners2d = boxes3d_utils.corners3d_to_corners2d(
            corners3d_camera, cam_intrinsic=cam_intrinsic
        )
        self.draw_corners(
            corners2d[keep],
            classes[keep],
            show_ori=True,
            bottom_indexes=[2, 3, 6, 7],
            **kwargs,
        )

    def draw_boxes3d_lidar(
        self, boxes3d_lidar, classes, lidar2cam, cam_intrinsic, method=2, **kwargs
    ):
        if len(boxes3d_lidar) == 0:
            return
        if method == 1:
            boxes3d_camera = boxes3d_utils.convert_boxes3d(
                boxes3d_lidar, "lidar", "camera", lidar2cam
            )
            corners3d_camera = boxes3d_utils.boxes3d_to_corners3d(
                boxes3d_camera, rot_axis=1
            )
            ...
            corners3d_lidar = boxes3d_utils.boxes3d_to_corners3d(
                boxes3d_lidar, rot_axis=2
            )
            corners3d_one = np.ones(
                [corners3d_lidar.shape[0], corners3d_lidar.shape[1], 1],
                dtype=corners3d_lidar.dtype,
            )
            corners3d_lidar_homo = np.concatenate(
                [corners3d_lidar, corners3d_one], axis=2
            )
