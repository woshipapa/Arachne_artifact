...
class BatchSamplerShard(BatchSampler):
    ...
    def __init__(
        ...
        # # my modify
        # self.process_index = self.process_index // 8
        self.split_batches = split_batches
        self.even_batches = even_batches
        self.batch_size = getattr(batch_sampler, "batch_size", None)
        self.drop_last = getattr(batch_sampler, "drop_last", False)
        # from vast.train.utils import logger
        # logger.info(f'num_processes : {self.num_processes}, process_index : {self.process_index}, batch_sampler.batch_size: {self.batch_sampler.batch_size}') 
    ...
    def _iter_with_no_split(self):
        ...
        for idx, batch in enumerate(self.batch_sampler):
            ...
            if idx % self.num_processes == self.num_processes - 1 and (
                ...
                # print(f'[Rank {torch.distributed.get_rank()}]   batch_to_yield : {batch_to_yield}, num_processes = {self.num_processes}, process_index = {self.process_index}')
...
def prepare_data_loader(
    ...
    sequnce_parallel_size: int = None
) -> DataLoader:
    """
...
#     logger.info(
#     f"Enter accelerate prepare_data_loader\n"
#     f"  - num_processes   : {num_processes}\n"
#     f"  - data_parallel_size         : {data_parallel_size}\n"
#     f"  - sequence_parallel_size         : {sequnce_parallel_size}"
# )
    ...
    # logger.info(f'num_processes : {num_processes}, process_index : {process_index}')
    # Sanity check
    if split_batches:
    ...
    if (num_processes != 1 or state.distributed_type == DistributedType.MEGATRON_LM) and not dispatch_batches:
        if isinstance(new_dataset, IterableDataset):
            ...
            SEQUENCE_SIZE = sequnce_parallel_size
            num_processes = int( num_processes / SEQUENCE_SIZE)
            new_batch_sampler = BatchSamplerShard(
                batch_sampler,
                num_processes=num_processes,
                process_index=process_index // SEQUENCE_SIZE,
                split_batches=split_batches,
    ...
    if dispatch_batches:
        ...
        # logger.info(f'Rank {torch.distributed.get_rank()} dataset :{new_dataset}, sampler = {new_batch_sampler}, batch_size = {dataloader.batch_size}')
