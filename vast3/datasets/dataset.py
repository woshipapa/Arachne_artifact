...
from teleai_data_tool.datasets.base_dataset import BaseDataset
from teleai_data_tool.datasets.file_dataset import FileDataset
...
def get_abs_path(abs_or_rel_path, data_dir=None):
    if os.path.isabs(abs_or_rel_path):
        ...
        for data_dir in data_dirs:
            abs_path = os.path.abspath(os.path.join(data_dir, abs_or_rel_path))
...
class Dataset(BaseDataset):
    def __init__(self, datasets, root_path=None):
    ...
    def load(cls, data_or_config):
        ...
        config_paths = [
            get_abs_path(config_path, data_path) for config_path in config_paths
        ]
        config_paths = sort_paths(config_paths)
        datasets = [load_dataset(config_path) for config_path in config_paths]
        return cls(datasets, root_path=data_or_config)
    ...
    def save(self, save_path, store_rel_path=True):
        ...
        if store_rel_path:
            config_paths = [
                get_rel_path(config_path, save_path) for config_path in config_paths
            ]
        config = {
            "_class_name": "Dataset",
    ...
    def _get_data(self, index):
        ...
        data_dict.update({"root_dir": self.root_path["data_path"]})
        if "data_index" in data_dict:
            data_index = data_dict["data_index"]
            ## 这里是临时代码, 在处理数据集时, prompts_embedding 长度和labels是对不上的，需要通过labels 中的prompt_data_index 去索引IMDB 的prompts embedding
            if "prompt_data_index" in data_dict:
                prompt_data_index = data_dict["prompt_data_index"]
                prompt_dataset_idx = -1
                if self.prompt_dataset is None:
                    # 默认prompt_embdding 是最后一行
                    self.prompt_dataset = self.datasets[-1]
                    self.datasets = self.datasets[:-1]
                data_dict.update(self.prompt_dataset[prompt_data_index])
        else:
            data_index = index
...
class ConcatDataset(BaseDataset):
    def __init__(self, data_path_list):
        ...
        self.datasets = [load_dataset(d) for d in data_path_list]
    ...
    def get_bucket_index_list(self):
        dataset_list = [d.datasets[0] for d in self.datasets]
        indices_dict = dict()
        count = 0
    ...
    def filter(self, mode, **kwargs):
        if mode.startswith("overall"):
            ...
            for i, dataset in enumerate(self.datasets):
                logging.info(
                    f"filter dataset {i} from {dataset.datasets[0].data_size} to {len(all_data_list[i])}"
                )
