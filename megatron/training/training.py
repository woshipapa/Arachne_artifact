    ...
    try:
        if rank == 0:
            if os.path.exists(resume_file):
                logger.info(f"\n🔎 [Auto-Resume] Rank0 检测到 OOM 恢复文件: {resume_file}")
        ...
        if resume_flag == 1 and resume_curr_iter >= 0:
            ...
            logger.info(
                f"🚀 [Rank {rank}] 从 curr_iter {new_curr_iter} "
                f"(Iteration {new_iteration}) 恢复训练"
            )
            ...
            logger.info(f"✅ [Rank {rank}] 未检测到崩溃记录，正常启动。")

        if rank == 0 and resume_flag == 1:
            os.remove(resume_file)

    except Exception as e:
        logger.error(f"⚠️ Auto-resume 失败，按默认方式启动: {e}")
        ...
        if os.environ.get("ENABLE_PROFILE_DIT_LAYER") == "1" or os.environ.get("PROFILE_TASK_TYPES") != "":
            ...
            PLANNER_PATH = f"t2v_flow/planner/task_yamls/vae_tasks_{1 << ( (iteration + per_iter_times*test_sp) // per_iter_times)}.yml"
                ...
                if os.environ.get('MAX_FRAMES', "") != "":
                    ...
                    PLANNER_PATH = f"t2v_flow/planner/generated_schedules/{os.environ.get('MODEL_TYPE')}/{os.environ.get('RESOLUTION')}/{os.environ.get('MAX_FRAMES')}/{os.environ.get('SCHEDULE_TYPE')}/schedule_{curr_iter}.yaml"
                else:
...
def simulate_data_loading_step(
    ...
    """
    一个用于模拟数据加载的函数，它只从 data_iterator 中获取数据，
    并将所有Rank的batch shape信息按顺序汇总到一个日志文件中。
    """
    rank = dist.get_rank()
