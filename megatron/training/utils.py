...
def average_losses_across_data_parallel_group(losses):
    ...
    # print(f"[Rank {torch.distributed.get_rank()}]average finished!-------")
...
def report_memory(name):
    ...
    args = get_args()
    if args.debug:
        from megatron.utils import log_first_rank
        log_first_rank(string)
...
def get_batch_on_this_tp_cp_rank_vast(data_iterator):
    ...
    def _broadcast(item):
        if item is not None:
           torch.distributed.broadcast(item, mpu.get_tensor_context_parallel_src_rank(), group=mpu.get_tensor_context_parallel_group())
    print(f'[Rank {torch.distributed.get_rank()}]  tensor_context_rank = {mpu.get_tensor_context_parallel_rank()}, tensor_rank = {mpu.get_tensor_model_parallel_rank()}, context_rank = {mpu.get_context_parallel_rank()}')
