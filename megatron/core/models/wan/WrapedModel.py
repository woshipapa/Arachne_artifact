from .DITPipeline import WanDiTPipeline
from .WanVAEPipeline import WanVAEPipeline
import torch
...
class TrainingWrapperModel(nn.Module):
    ...
    以适配Megatron的训练框架。
    """
    def __init__(self, wan_config, dit_model_config):
        """
        在容器内部，实例化所有需要的子模型。
        ...
        self.vae_pipeline = WanVAEPipeline(wan_config)

        # 2. 实例化 DiT Pipeline
        # DiT是我们需要训练的核心模型。
        # 注意: 这里的配置需要根据您的实际情况传入
        self.dit_pipeline = WanDiTPipeline(wan_config, dit_model_config)
