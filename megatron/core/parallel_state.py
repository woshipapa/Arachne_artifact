...
def use_custom_group(group):
    ...
    try:
        ...
        print(f"[Rank {rank_info}] caught a CUDA out-of-memory exception. Triggering automatic recovery.")
        print(f"   error: {e}")
        ...
        try:
            ...
            if is_master:
                print(f"[Master] writing the crashing iteration {current_step} to: {RESUME_FILE_PATH}")
            ...
            print(f"[Error] failed to record the resume point: {record_err}")

        try:
            from my_utils import global_timer
            print(f"   running global_timer.dump()...")
            global_timer.dump()
        except:
            pass
            
        print(f"Exiting with status code {EXIT_CODE_OOM} to request a restart from the shell...")
        print("="*60 + "\n")
