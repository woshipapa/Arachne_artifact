...
def forward_step(
    ...
    # print(f'forward_step config = {config}')
    # from my_utils.my_utils import MyTimer
    # fs_timer = MyTimer(use_cuda=True, tag="forward_step(wrap forward_step_func)")
    # fs_timer.start()
...
# ) as prof:
    ...
    # fs_timer.stop()        
    # print(f'rank {torch.distributed.get_rank()} loss = {output_tensor}, loss_func = {loss_func}')
    ...
    # print(f'rank {torch.distributed.get_rank()} loss = {forward_data_store}')
...
def backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config):
    ...
    backward_str = f"backward_bs_{os.environ.get('bs')}_f_{os.environ.get('frames')}_h_{os.environ.get('height')}_w_{os.environ.get('width')}_sp_{mpu.get_context_parallel_world_size()}"
    global_timer.start(backward_str)
    # Backward pass.
    ...
    global_timer.stop(backward_str)
    ...
    *,
    ...
    with no_sync_func():
        for i in range(num_microbatches - 1):
            # with MyTimer(tag=f"forward_mb{i}", use_cuda=True):
            ...
            if not forward_only:
                # with MyTimer(tag=f"backward_mb{i}", use_cuda=True):
                    global_timer.start(f"backward_mb{i}")
                    backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config)
                    global_timer.stop(f"backward_mb{i}")

    # Run computation for last microbatch out of context handler (want to
    # synchronize gradients).
    # with MyTimer(tag=f"forward_mb{num_microbatches-1}", use_cuda=True):                
    output_tensor = forward_step(
            forward_step_func,
            data_iterator,
            model,
    ...
    if not forward_only:
        # with MyTimer(tag=f"backward_mb{num_microbatches-1}", use_cuda=True):
            # global_timer.start(f"backward_mb{num_microbatches-1}")
            backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config)
            # global_timer.stop(f"backward_mb{num_microbatches-1}")

    if config.timers is not None:
        config.timers('forward-backward').stop()

    # traditional static parallelism enable this branch
