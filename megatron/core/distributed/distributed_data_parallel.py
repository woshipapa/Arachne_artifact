...
class DistributedDataParallel(MegatronModule):
    ...
    Args:
        ...
        force_bucketing: If true, keep per-bucket grad_data tensors regardless of
            overlap_grad_reduce. Used by custom heterogeneous grad sync (sync_grads_with_plan)
            which pipelines AllReduce and Broadcast at bucket granularity without relying on
            Megatron's native overlap_grad_reduce hook.
    ...
    def __init__(
        ...
        # Determine effective bucket_size.
        # Bucketing is enabled when:
        #   (a) overlap_grad_reduce is True (Megatron native overlap), OR
        #   (b) force_bucketing is True (custom grad sync pipelines at bucket granularity)
        # Bucketing is disabled when:
        #   - disable_bucketing is True (e.g. model chunks beyond the first in interleaved PP)
        #   - pp_rank > 0 (grad communication on non-first stages is not on the critical path)
        if not self.overlap_grad_reduce and not force_bucketing:
            bucket_size = None
