...
class NCCLLibrary:
    exported_functions = [
        ...
        # AMem extended NCCL APIs
        Function("ncclPause", ncclResult_t, [ctypes.POINTER(ncclComm_t)]),
        Function("ncclResume", ncclResult_t, [ctypes.POINTER(ncclComm_t)]),
        Function("ncclMemStats", ncclResult_t, []),
        Function("ncclSetGroupID", ncclResult_t, [ctypes.c_int]),
        Function("ncclGetGroupID", ncclResult_t, [ctypes.POINTER(ctypes.c_int)]),
    ...
    def __init__(self, so_file: Optional[str] = None):
        ...
        if so_file not in NCCLLibrary.path_to_dict_mapping:
            ...
            if hasattr(self.lib, "ncclAllToAll"):
                exported_functions.extend(NCCLLibrary.exported_functions_alltoall)
            if hasattr(self.lib, "ncclPause"):
                exported_functions.extend(NCCLLibrary.exported_functions_amem)
            for func in exported_functions:
                # 这里访问.so中是否有定义的export functions
    ...
    def ncclPause(self, comm: Optional[ncclComm_t] = None) -> None:
        if comm is None:
            self.NCCL_CHECK(self._funcs["ncclPause"](None))
        else:
            self.NCCL_CHECK(self._funcs["ncclPause"](ctypes.byref(comm)))


    def ncclResume(self, comm: Optional[ncclComm_t] = None) -> None:
        if comm is None:
            self.NCCL_CHECK(self._funcs["ncclResume"](None))
        else:
            self.NCCL_CHECK(self._funcs["ncclResume"](ctypes.byref(comm)))


    def ncclMemStats(self) -> None:
        self.NCCL_CHECK(self._funcs["ncclMemStats"]())


    def ncclSetGroupID(self, gid: int) -> None:
        self.NCCL_CHECK(self._funcs["ncclSetGroupID"](gid))


    def ncclGetGroupID(self) -> int:
        gid = ctypes.c_int()
        self.NCCL_CHECK(self._funcs["ncclGetGroupID"](ctypes.byref(gid)))
        return gid.value
